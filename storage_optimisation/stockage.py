...
class Stockage():
    ...
    def tunnel_max(self):
        L=[] # Construire directement à la bonne taile 
        def chapiteau_max(i,dirac):
            ...
            return serie        
        for i in self.threshold_con.keys():
            L .append(self.Vmax*chapiteau_max(i -self.index_i,self.threshold_con[i][1]))
        # plt.plot(self.dates,np.array(L).transpose())
        # plt.plot(self.dates,np.zeros(self.N))
        # plt.title('tete')
    ...
    def tunnel_min(self):
        ...
        def chapiteau_min(i,dirac):
            ...
            return serie        
        for i in self.threshold_con.keys():
            L .append(self.Vmax*chapiteau_min(i -self.index_i,self.threshold_con[i][0]))
        L = np.array(L)
    ...
    def tunnel(self):
        plt.plot(self.dates,self.tunnel_max())
        plt.plot(self.dates,self.tunnel_min())
