...
class Stockage():
    ...
    def volume_vect(self):
        ...
        """
        #
        v_vect = self.Vmax * np.dot(self.m.triang_inf,
                                    self.evolution) + self.Vinit * np.ones(self.N)
        return v_vect
    ...
    def sout_correction(self, v: np.array):
        ...
        for i in range(1, len(self.sout_list)):
            if stock_level <= self.sout_list[i][0]:  # A rendre +modualir
                return 1 / self.D_nom_sout * (self.sout_list[i - 1][1] +
                                              (self.sout_list[i][1] - self.sout_list[i - 1][1]) /
                                              (self.sout_list[i][0] - self.sout_list[i - 1][0]) *
                                              (stock_level - self.sout_list[i - 1][0]))
        return 0
    ...
    def sout_corrige(self, X: np.array):
        ...
        Return :
            - np.array() : Maximal percentage of the Vmax rackable in the storage
            each day
        """
        V = self.Vmax * np.dot(self.m.triang_inf, X) + \
            self.Vinit * np.ones(self.N)
    ...
    def inj_correction(self, v):
        ...
        for i in range(1, len(self.inj_list)):
            if stock_level <= self.inj_list[i][0]:  # A Rendre + modulaire
                return 1 / self.D_nom_inj * (self.inj_list[i - 1][1] +
                                             (self.inj_list[i][1] - self.inj_list[i - 1][1]) / (
                    self.inj_list[i][0] - self.inj_list[i - 1][0]) *
                    (stock_level - self.inj_list[i - 1][0]))
        return 0

    def inj_corrige(self, X):
        """
        Function that for an evolution time serie determines the maximal injection
        each day
        Parameters :
            - Effective evolution in the storage
        Return :
            - np.array() : Maximal percentage of the Vmax injectable in the storage
            each day
        """
        V = self.Vmax * np.dot(self.m.triang_inf, X) + \
            self.Vinit * np.ones(self.N)
    ...
    def threshold(self):
        ...
        threshold = {}
        for month in self.months_con:  # On itère sur les contraintes
            if datetime.datetime(
                    begin_year,
                    int(month),
                    1) < begin:  # test si cela commence après
                if datetime.datetime(
                        begin_year + 1,
                        int(month),
                        1) < end:  # test si cela finit avant
                    index_number = []
                    current_search_day = datetime.datetime(
                        begin_year + 1, int(month), 1)
                    while len(index_number) == 0:
                        index_number = self.data[self.data['Day'] ==
                                                 current_search_day].index.values.astype(int)
                        current_search_day = current_search_day - \
    ...
    def tunnel_max(self):
        ...
        and the vectors returned by Stockage.chapiteau_max. Where Stockage.chapiteau_max
        returns the maximal behaviour authorized by the constraints,
        meaning the limit behaviour to respect the constraint of the beginning
        of some months.
            e-g : Les positions autorisés en ne faisant que acheter avant et que
            vendre après le 1 aout, tout en étant à 90 % de remplissage ce jour là.
        Parameters:
            - self
        Return :
            - Vector of minimum volume each day  : np.array
        ...
        L = np.zeros((len(self.threshold_con.keys()), self.N)
                     )  # Construire directement à la bonne taile

        def chapiteau_max(i: int, dirac: float):
            """Function that computes the maximum feasible behaviour, starting
            symetrically from a day of legal obligation
            Parameters :
                - self
            ...
            for pas in range(1, self.N):
                if i + pas <= i_max:  # verify it is still in the limits
                    ...
                    serie[i + pas] = serie[i + pas - 1] + \
                        self.inj_correction(self.Vmax * serie[i + pas - 1])
                if i - pas >= i_min:
                    # rack as much gas as possible moving backward
                    serie[i - pas] = serie[i - pas + 1] + \
                        self.sout_correction(self.Vmax * serie[i - pas + 1])
            return serie
        for index, j in enumerate(self.threshold_con.keys()):
            L[index] = (
                ...
                chapiteau_max(
                    j -
                    self.index_i,
                    self.threshold_con[j][1]))
        # add a line of Vmax (superior limit)
    ...
    def tunnel_min(self):
        ...
        def chapiteau_min(i, dirac):
            """Function that computes the minimum feasible behaviour,
            starting symetrically from a day of legal obligation
            Parameters :
                - self
                - index : position of the threshold
                - dirac : value of the threshold
        ...
        for index, j in enumerate(self.threshold_con.keys()):
            L[index] = (
                ...
                chapiteau_min(
                    j -
                    self.index_i,
                    self.threshold_con[j][0]))
        L = np.array(L)
        # add a line of zeros (inferior limit)
        L = np.vstack((L, np.zeros(self.N)))
        return np.array(L).max(axis=0)

    def tunnel(self):
        """ Function that creates the attribute of the optimization
...
def main2():
    ...
    data_select.loc[:, 'Day'] = pd.to_datetime(
        data_select['Day'], format='%Y-%m-%d')
