...
#Initial formatting so the module can work correctly specifically for the PowerNext file
...
df.to_csv('../Data/New_Power_Next_spot.csv', date_format = '%Y-%m-%d',
          columns=['Day', 'Price'], index=False)


#Now we can work with our newly formatted file!
path_new = '../Data/New_Power_Next_spot.csv'
diff = DiffusionSpot(path_new, path2, forward_diffusion=False) #Diffusing around historical mean 
...
tab, moyenne, long_term_means, n = diff.multiple_price_scenarios(start_date_long,
...
print(f'mean_reversion_sum = {diff.mean_reversion_sum},\
      mean_reversion_win =  {diff.mean_reversion_win},\
      vol_sum = {diff.vol_sum}, vol_win = {diff.vol_win},\
      long_term_vol = {diff.long_term_vol}')
