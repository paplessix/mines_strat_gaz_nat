...
class Browser:
    ...
    def scraper_iterator(
            self,
            specific_type=False,
            link="https://www.powernext.com/spot-market-data",
            bloc_number=0):
        ...
        for gnl_type in self.gnl_types:
            if (not specific_type) or (gnl_type.text in specific_type):
                ...
                # Let the time for the website to load the graph. For more
                # explanation see doc.
                time.sleep(5)
                ...
                pos_abs_middle_x = (max_x - min_x) / 2 + \
                    chart_container.rect['x'] + min_x
                pos_abs_middle_y = (max_y - min_y) / 2 + \
                    chart_container.rect['y'] + min_y
                # Move to the left limit of the graph
                offset_x = pos_data[0][0] + \
                    chart_container.rect['x'] - pos_abs_middle_x
                offset_y = pos_data[0][1] + \
                    chart_container.rect['y'] - pos_abs_middle_y
                ...
                while active_pos_x <= max_x - 2:
                    ...
                    if date == last_date:
                        ...
                        last_date = date
                        data_list_append(
                            prices, price, dates, date, prediction_types, prediction_type)
